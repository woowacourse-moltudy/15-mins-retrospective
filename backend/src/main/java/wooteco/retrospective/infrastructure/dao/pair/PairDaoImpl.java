...
public class PairDaoImpl implements PairDao {
    ...
    public Optional<Pairs> findByDateAndTime(LocalDate date, ConferenceTime conferenceTime) {
        List<AttendanceWithGroupId> members = jdbcTemplate.query(
            getFindByDateAndTimeSql(),
            rowMapperForFindByDateAndTime(conferenceTime),
            date,
    ...
    private RowMapper<AttendanceWithGroupId> rowMapperForFindByDateAndTime(ConferenceTime conferenceTime) {
        return (rs, rn) -> {
            Long groupId = rs.getLong("group_id");
            Attendance attendance = createAttendance(conferenceTime, rs);

            return new AttendanceWithGroupId(groupId, attendance);
        };
    }

    private Attendance createAttendance(ConferenceTime conferenceTime, java.sql.ResultSet rs) throws SQLException {
        Long attendanceId = rs.getLong("attendance_id");
        LocalDate date = rs.getObject("date", LocalDate.class);

        return new Attendance(attendanceId, date, createMember(rs), conferenceTime);
    }
    ...
    private String getFindByDateAndTimeSql() {
        return "SELECT " +
            ...
            "WHERE A.date=? and A.conference_time_id = " +
            "(SELECT id FROM CONFERENCE_TIME WHERE conference_time = ?)";
    }
